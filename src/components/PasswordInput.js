...
const PasswordInput = (props) => {
  const { label, name, text, error = "", control } = props;
  ...
  return (
    <div>
      <div className="flex flex-col mb-4">
        ...
        <div
          className={`relative flex items-center w-full px-6 py-4 text-sm font-medium border text-text2 rounded-xl dark:border-darkStroke ${
        ...
        >
          ...
          />
          {error.length > 0 && (
            <span className="absolute text-sm font-medium pointer-events-none top-2/4 -translate-y-2/4 left-6 text-error">
              {error}
            </span>
          )}
